public class Draw {
     ...
     */
    public void drawLine(int x1, int y1, int x2, int y2) { /* implementation not shown */ }
    ...
    public void drawSquare(int x, int y, int len) {
        int len = Math.min(Math.min(10-x,y),len);
        drawLine(x, y, x+len, y);
        drawLine(x, y, x, y-len);
        drawLine(x+len, y, x+len, y-len)
        drawLine(x, y-len, x+size, y-len);
        System.out.println("len length = " + len + ",area = " + len*len);
    }
}
